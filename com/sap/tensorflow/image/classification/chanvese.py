...
def convergence(p_mask, n_mask, thresh, c):
    #diff = p_mask - n_mask
    diff = np.subtract(p_mask, n_mask, dtype=np.float)
...
def invokeChanvese(originalImage, filteredImage, infilename, filePathToSave):
    ...
    #seg is a tuple of 3 elements
    #seg[0] is a boolean array which is the final mask
    
    seg=chanvese(greyImg, mask, max_its=1000, display=False, alpha=1)
    
    # convert the boolean mask to integer of 0 and 1 , 0 - background , 1 - foreground
    finalMask = seg[0] * 1;
